...
from pydsdb import get_dataset
import scanpy as sc
...
def common_genes (embedding_key, PATH):
    dfs=[]
    GENEs = []
    for i in range(10):
        STR = "e_dist_500NTCs_"+embedding_key+"_"+str(i+1)
        path  = os.path.join(PATH, f'{STR}.csv')
...
def evaluate(
    ...
    plt.title("KS plot comparing embedding similarity of gene sets vs random sets (DLDL1)")
