...
def energy_distance_test(
    ...
    ### Random choice of 500 subsampling of NTC ####
    
    def Rand_subsample(x,y):
        y_r = y[np.random.choice(y.shape[0], 500, replace=False)]
        z_r = np.concatenate([x, y_r])
        dzz_r = pairwise_distances(z_r)
    
        def e_dist_r(ix1, ix2):
            return (
                dzz_r[np.ix_(ix1, ix2)].mean()
                - 0.5 * dzz[np.ix_(ix1, ix1)].mean()
                - 0.5 * dzz[np.ix_(ix2, ix2)].mean()
            )
    
        return e_dist_r(*np.split(np.random.permutation(len(z_r)), [len(x)]))
