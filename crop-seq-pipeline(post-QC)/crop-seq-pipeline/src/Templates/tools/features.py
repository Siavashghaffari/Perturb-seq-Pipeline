...
class Features (object):
    ...
    def __init__(self, DatasetID, auth, **kwargs):
        ...
        self.Norm_param = kwargs.pop('Norm_param', dict({'use_DSDB':False}))
        ...
        self.ED_param = kwargs.pop('ED_param', dict({'pVAL':0.05, 'n':500, 'embeddings_key':"X_pca", 'i':1}))
        self.MV_param = kwargs.pop('MV_param', dict({'embeddings_key':"X_pca", "transcriptoprint":False}))
        
        if self.MV_param["transcriptoprint"]==True:
            self.MV_transcriptoprint = "Phonetype_genes"
        else:
            self.MV_transcriptoprint = "all_genes"
        ...
        self.norm_notebook = os.path.join(self.Notebooks_HOME, "Norm_control.ipynb")
        ...
        self.norm_notebook_out = f'{self.Notebooks_Savepath}/{self.DatasetID}_NormControl.ipynb'
        ...
        self.multivariate_notebook_out = f'{self.Notebooks_Savepath}/{self.DatasetID}_Multivariate_{self.MV_param["embeddings_key"].split("_",1)[-1]}_{self.MV_transcriptoprint}.ipynb'
        ...
        self.Dataset_HOME_ft = f'{self.Dataset_HOME}/Fisher_test'
        """
        try:
        ...
        self.Dataset_HOME_norm = f'{self.Dataset_HOME}/test_norm'
        try:
            os.mkdir(self.Dataset_HOME_norm)
        except OSError as error:
            print(error)
        ...
        self.Dataset_HOME_ed = f'{self.Dataset_HOME}/Energy_Distance'
        try:
        ...
        self.Dataset_HOME_em = f'{self.Dataset_HOME}/Embeddings'
        try:
            os.mkdir(self.Dataset_HOME_em)
        except OSError as error:
            print(error)
        ...
        # Create an AnnData for energy distance
        raw_embedding = f'{self.Dataset_HOME}/qc_embedding.h5ad'
        self.ED_file = f'{self.Dataset_HOME_ed}/e_dist_all.h5ad'
        if os.path.exists(self.ED_file):
            pass
        else:
            try:
                shutil.copy(raw_embedding,self.ED_file)
        ...
        if len(self.Embeddings_param["subsample_dic"])>0:
            self.Dataset_HOME_Subsample = f'{self.Dataset_HOME}/Subsample_{list(self.Embeddings_param["subsample_dic"].items())[0][0]}_{list(self.Embeddings_param["subsample_dic"].items())[0][1]}_{list(self.Embeddings_param["subsample_dic"].items())[1][0]}_{list(self.Embeddings_param["subsample_dic"].items())[1][1]}'
            try:
                os.mkdir(self.Dataset_HOME_Subsample )
            except OSError as error:
                print(error)
            embeddings_in = f'{self.Dataset_HOME_Subsample}/qc_embedding.h5ad'
            embeddings_out = f'{self.Dataset_HOME_Subsample}/qc_embedding_{list(self.Embeddings_param["subsample_dic"].items())[0][0]}_{list(self.Embeddings_param["subsample_dic"].items())[0][1]}_{list(self.Embeddings_param["subsample_dic"].items())[1][0]}_{list(self.Embeddings_param["subsample_dic"].items())[1][1]}.h5ad'
 
            self.Dataset_HOME_ed = f'{self.Dataset_HOME_Subsample}/Energy_Distance'
            try:
                os.mkdir(self.Dataset_HOME_ed)
            except OSError as error:
                print(error)
            ...
            if os.path.exists(self.ED_file):
                ...
                try:
                    shutil.copy(embeddings_out,self.ED_file)
                except FileNotFoundError as error:
                    print (error)
        else:
        ...
        try:
            ...
            self.your_experiment = ''
            self.title = "Recursion DLD1 Library4 Screen Day 5- combination of NGS 5570 and NGS 5704"
        ...
        self.embeddings_parameters = dict(
            ...
            Dataset_Home = self.Dataset_HOME,
            out_norm = self.Dataset_HOME_norm,
            out_embeddings = self.Dataset_HOME_em,
                raw_qc=embeddings_in,
        raw_embedding =embeddings_out)
        
        
        self.norm_parameters = dict(
                DatasetID=self.DatasetID,
                DEV = self.DEV,
                test_DatasetID = self.test_DatasetID,
                DS_test = self.DS_test,
                ...
                author = self.author,
                organism=self.gene_param["organism"],
            use_DSDB = self.Norm_param["use_DSDB"],
            raw_qc=f'{self.Dataset_HOME}/raw_qc.h5ad',
        out_dir = self.Dataset_HOME_norm)
        ...
        self.ED_parameters = dict(
            ...
            i = self.ED_param["i"],
            ED_file = self.ED_file,
            key_uns = f'perGene_{self.ED_param["embeddings_key"].split("_",1)[-1]}_{self.ED_param["i"]}',
            out_csv = f'{self.Dataset_HOME_ed}/e_dist_{self.ED_param["n"]}NTCs_{self.ED_param["embeddings_key"].split("_",1)[-1]}_{self.ED_param["i"]}.csv'
        )
        ...
        self.ED_parameters_P = dict(
            ...
            i = self.ED_param["i"],
            ED_file =  self.ED_file,
            key_uns = f'perPerturbations_{self.ED_param["embeddings_key"].split("_",1)[-1]}_{self.ED_param["i"]}',
            out_csv = f'{self.Dataset_HOME_ed}/e_dist_perPerturbations_{self.ED_param["n"]}NTCs_{self.ED_param["embeddings_key"].split("_",1)[-1]}_{self.ED_param["i"]}.csv'
        )
        ...
        self.MV_parameters = dict(
                ...
                author = self.author,
                organism=self.gene_param["organism"],
            embeddings_key = self.MV_param["embeddings_key"],
            transcriptoprint = self.MV_param["transcriptoprint"],
            MV_file = f'{self.Dataset_HOME_ed}/e_dist_all.h5ad',
            path=self.Dataset_HOME_ed
        )
    ...
    def Norm_control (self):
        pm.execute_notebook(
            self.norm_notebook,
            self.norm_notebook_out,
            parameters = self.norm_parameters,
            kernel_name='gpuy310'
        )
        if self.Reports_format=="html":
            command = f'jupyter nbconvert {self.norm_notebook_out} --to html'
            subprocess.run(command, shell=True)
            os.remove(self.norm_notebook_out)
        elif self.Reports_format=="pdf":
            command = f'jupyter nbconvert {self.norm_notebook_out} --to pdf'
            subprocess.run(command, shell=True)
            os.remove(self.norm_notebook_out)
