...
def concat_norm_matrix(HVG, folder_paths, HVG_file=False):
    """
    This function concatenates all the sxmnall anndta which were previously created by Norm_control module
    HVG is a list of Highly variable genes passed through this function if variable HVG_file is False, in case of HVG_file=True
    a path to where the HVG pickle file is stored is passed through the function, HVG_file=Boolean
    folder_path is a list of folder paths which small anndatas locate
    """
    if HVG_file:
        with open(HVG, "rb") as fp:   # Unpickling
            HVG = pickle.load(fp)
    ...
    try:
        ...
        adata.obs['total_counts'] = adata.obs['total_counts'].astype("Int64")
        adata.obs['total_counts_mt'] = adata.obs['total_counts_mt'].astype("Int64")
        adata.obs['total_counts_ribo'] = adata.obs['total_counts_ribo'].astype("Int64")
        adata.obs['qc_pass'] = adata.obs['qc_pass'].astype("bool")
        adata.obs['10Xrun'] = adata.obs['10Xrun'].astype("str")
        adata.obs['gRNA_library_MOI'] = adata.obs['gRNA_library_MOI'].astype("str")
    except KeyError:
        pass
    adata.write("/gstore/scratch/u/ghaffars/Dataset/Norm_control/Sublib4/test.h5ad")
    return(adata)



def concat_PCA_norm(adata):
    #sc.pp.scale(adata, max_value=10)
    adata.X = np.nan_to_num(adata.X, copy=False)
    sc.tl.pca(adata)
    centered_embedding_adata(
            adata,
            'X_pca',
            'gene_symbol',
      'DemuxAssignment_crispr',
            "NTC",
        pool_ntcs=False)
    run_sphering_transform(adata, query="gene_symbol == 'NTC'", reg_param=1e-3)
    return adata