...
## Script to run
script=/gstore/home/ghaffars/Cumulus/crc_dld1_sublib1_bdev/crop-seq-pipeline/src/glmGamPoi_NE/05.Run_glmGamPoi_10000.R



## Input file split by arget
input_directory=/gstore/scratch/u/ghaffars/glmGamPoi/sublib1_bdev_NE/data/

## Gene File - Info of Gene Names - Aka Rows/Features 
gene_file=/gstore/scratch/u/ghaffars/glmGamPoi/sublib1_bdev_NE/data/remove_pos_cont_gene_data.csv

# # Output Directory where to store results

output_dir=/gstore/scratch/u/ghaffars/glmGamPoi/sublib1_bdev_NE/results/
...
if [ ! -e ${output_dir}unweightedglmGamPoi_gembatch_results/glm_unweighted_target_${target}_top10000_sublib124union_genes.rds ]; then
    ...
    ## Submit job
    Rscript ${script} -i ${input_directory} -g ${gene_file} -o ${output_dir} -t ${target} -c ${control}
